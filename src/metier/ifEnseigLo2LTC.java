...
public class ifEnseigLo2LTC extends JInternalFrame {
	/*
	 * 
	 * 
	 */
	...
	btnAjouter.addActionListener(new ActionListener() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(checkInput() == true &&  !txIDEnv.getText().toString().equals("") && !comboAnso.getSelectedItem().toString().equals("Choix")){
				...
				String sql = "insert into enseignantelltc ( cours, nomEns, clsEns, idEns) values ( ?,? , ?,?)";
				...
				String sql2 = "insert into matparclasseltc ( idMClsEns , crTitre, clsNom, anne_scol) values ( ?,? , ?,?)";
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(!txIDEnv.getText().toString().equals("") && !textid.getText().toString().equals("")){
				try {
					String sql =" update enseignantelltc set nomEns =?, clsEns = ?,  cours =?,idEns = ?  where idMEns =?";
	...
	btnSupprimer.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
            String sql = "DELETE FROM `enseignantelltc` WHERE `idMEns`=?";
	...
	panel.setLayout(null);
		...
		comMatiere.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(!comMatiere.getSelectedItem().equals("Choix")){
					String sql = "select cr_id from coursltc where cr_titre = '"+comMatiere.getSelectedItem().toString()+"'";
					try {
						prepared = cnx.prepareStatement(sql);
		...
		comClasse.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String sql = "select id_classe from classeltc where cls_nom = '"+comClasse.getSelectedItem().toString()+"'";
				try {
					prepared = cnx.prepareStatement(sql);
					resultat = prepared.executeQuery();
					if(resultat.next()){
			...
			table_2 = new JTable();
				...
				table_2.addMouseListener(new MouseAdapter() {
					   ...
					   /////////////
						...
						String sql = "select * from  enseignantelltc where idMEns ='"+id+"' ";
						...
						String sql2 = "select * from  enseignantelltc where cours ='"+id2+"' ";
						try {
							prepared = cnx.prepareStatement(sql2);
							resultat = prepared.executeQuery();
						...
						String sql3 = "select * from  enseignantelltc where clsEns ='"+id3+"' ";
						try {
	...
	String sql = "SELECT idMEns, cours,clsEns , nomEns FROM enseignantelltc ";
		try {
			prepared = cnx.prepareStatement(sql);
			resultat = prepared.executeQuery();
